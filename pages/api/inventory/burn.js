...
export default async function handler(req, res) {
  console.log('🚀 Функция burn вызвана');
  ...
  if (!steamId || !itemId) {
    console.log('❌ Неверные данные: отсутствует steamId или itemId');
  ...
  try {
    ...
    if (!doc.exists) {
      console.log('❌ Пользователь не найден:', steamId);
    ...
    console.log('🔍 Ищем скин с ID:', itemId);
    ...
    console.log('✅ Скин найден, индекс:', itemIndex);
    console.log('🗑️ Удаляем скин:', inventory[itemIndex].name);

    inventory.splice(itemIndex, 1);
    await userRef.update({ inventory: inventory });

    console.log('✅ Скин удалён. Осталось скинов:', inventory.length);

    return res.status(200).json({ success: true, remaining: inventory.length });
